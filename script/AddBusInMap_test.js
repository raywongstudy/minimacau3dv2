...
// 2. 巴士 处理巴士位置的更新
async function handleBusPositionUpdates(layer, response_bus_data, traffic_data) {
    ...
    await updateLayerCubes(layer, currentBusData, traffic_data);
...
// 2. 巴士 更新图层中的巴士立方体
async function updateLayerCubes(layer, currentBusData, traffic_data) {
    ...
    for (const busData of currentBusData) {  // 遍历最新API的数据
        ...
        for (const newBusInfo of busInfoList) {  // 遍历每个巴士信息
            ...
            const updatedCube = updateOrAddCube(layer, newBusInfo, cube_item_dir, traffic_data); 
            if (updatedCube) {
...
// 2. 巴士 更新或添加巴士立方体
function updateOrAddCube(layer, busInfo, dir, traffic_data) {
    ...
    // 如果找到了现有的立方体，更新其信息
    if (existingCube) {
        updateExistingCube(existingCube, busInfo, layer.longitude, layer.latitude, traffic_data);
...
// 2. 巴士 更新已存在的立方体
function updateExistingCube(cube, newBusInfo, longitude, latitude, traffic_data) {
    ...
    // 获取两个位置之间的路线点
    const pointsRange = getPointsBetweenRange(
        cube.userData.source_position,
        [parseFloat(newBusInfo.longitude), parseFloat(newBusInfo.latitude)],
        traffic_data
    );

    // 计算每个阶段的位置增量
    const animation_number = 50;  // 每段的动画帧数，可以根据需要调整
    const deltaPositions = calculateDeltaPositions(pointsRange, animation_number, longitude, latitude);

    // 设置用户数据，用于在 updateCubePosition 中使用
    cube.userData.deltaPosition = deltaPositions;  // 存储位置增量
    cube.userData.animation_number = Array(deltaPositions.length).fill(animation_number);  // 存储对应的动画帧数
...
function GenAllCustomLayer(map, route_elements, options = {}) { 
    ...
    const customLayer = {
        ...
        updateBusPositions: async function (response_bus_data, traffic_data) {
            // 調用 handleBusPositionUpdates 函數來處理巴士位置的更新
            await handleBusPositionUpdates(this, response_bus_data, traffic_data);
        }
    };
...
function calculateDeltaPositions(pointsRange, animation_number, longitude, latitude) {
    ...
    if (pointsRange.length > 0) {
        for (let i = 0; i < pointsRange.length - 1; i++) {
            ...
            const deltaY = (end[0] - start[0]) / animation_number;
            const deltaX = (end[1] - start[1]) / animation_number;
            deltaPositions.push([deltaY, deltaX]);
        }
    }
...
function calculateDistanceInDirection(lon1, lat1, lon2, lat2) {
    ...
    const distanceLon = dLon * 111194 * Math.cos(avgLat * Math.PI / 180); // 经度方向上的实际移动距离（米）
    return [distanceLat, distanceLon];// 返回以米为单位的结果
}

// 4. 功能 获取范围内的点（根据实际情况实现）
function getPointsBetweenRange(pointA, pointB, traffic_data) {
    ...
    const coordinateTuples = traffic_data.coordinate.map(coord => [parseFloat(coord[0]).toFixed(8), parseFloat(coord[1]).toFixed(8)]);

    const indexA = coordinateTuples.findIndex(coord => coord[0] === pointA[0] && coord[1] === pointA[1]);
    const indexB = coordinateTuples.findIndex(coord => coord[0] === pointB[0] && coord[1] === pointB[1]);
    ...
    if (indexA === indexB) {
        return [coordinateTuples[indexA]];
    } else {
        return coordinateTuples.slice(Math.min(indexA, indexB), Math.max(indexA, indexB) + 1);
    }
}
...
// 0. 主程式 添加巴士到地图上----------------------------------------------------------------------
async function AddBusInMap(map, filter_bus_lists = [], bus_api_link, traffic_data) {
    ...
    customLayers.forEach(layer => layer.updateBusPositions(response_bus_data, traffic_data));
    // 每隔5秒更新一次巴士位置
    setInterval(async () => {
        const response_bus_data = await fetch(bus_api_link).then(response => response.json());
        customLayers.forEach(layer => layer.updateBusPositions(response_bus_data, traffic_data));
